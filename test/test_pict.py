...
def _make_pv(fmt, sys_len, sub_len, prop_len, suffix, start_char):
    ...
    suffix_str = {"none": "", "-SP": "-SP", "-RB": "-RB", "-S": "-S", "-RBV": "-RBV", "-R": "-R"}[suffix]
...
class TestPICTFormat:
    ...
    @pytest.mark.parametrize("fmt,sys_len,sub_len,expect_format_valid", [
    ...
    @pytest.mark.parametrize("sys_len,sub_len,expect_elem_error", [
...
class TestPICTPropertyLength:
    ...
    @pytest.mark.parametrize("prop_len,suffix,expect_error,expect_warning", [
        ...
        # At hard limit (25)
        (25, "none", False, False),    # exactly 25 = OK (no error, may have PROP-2-WARN)
        (26, "none", True, False),     # 26 = PROP-2 error
        # Suffix excludes from length calculation (prop_len = total including suffix)
        (23, "-SP", False, False),     # total 23, effective 20 (23-3) = at recommended limit, no warning
        (23, "-RB", False, False),     # total 23, effective 20 (23-3) = at recommended limit, no warning
        (24, "-SP", False, True),      # total 24, effective 21 (24-3) = PROP-2-WARN (>20)
        (28, "-SP", False, True),      # total 28, effective 25 (28-3) = at hard limit, PROP-2-WARN
        (29, "-SP", True, False),      # total 29, effective 26 (29-3) = over hard limit, error
    ...
    def test_property_length_with_suffix(self, prop_len, suffix, expect_error, expect_warning):
        pv = _make_pv("full", 3, 3, prop_len, suffix, "uppercase")
        comp = parse_pv(pv)
        assert comp is not None
        msgs = check_all_rules(comp)
        if expect_error:
            assert _has_severity(msgs, Severity.ERROR), f"Expected error for prop_len={prop_len}, suffix={suffix}"
        if expect_warning and not expect_error:
            assert _has_severity(msgs, Severity.WARNING), f"Expected warning for prop_len={prop_len}, suffix={suffix}"


class TestPICTStartChar:
    """Property start character combinations."""

    @pytest.mark.parametrize("start_char,expect_error,expect_warning", [
...
class TestPICTConfusable:
    ...
    @pytest.mark.parametrize("prop_a,prop_b,expect_confusable", [
        # No confusion
    ...
    def test_confusable_pairs(self, prop_a, prop_b, expect_confusable):
        ...
        assert has_conflict == expect_confusable, \
...
class TestPICTCrossParameter:
    ...
    @pytest.mark.parametrize("fmt,prop_len,suffix,start_char", [
...
class TestPICTEffectiveLength:
    ...
    @pytest.mark.parametrize("prop,expected_len", [
        ("Temperature", 11),
