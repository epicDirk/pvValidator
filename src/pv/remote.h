...
public:
  ...
  virtual void startMessage(epics::pvData::int8 command,
                            std::size_t ensureCapacity,
                            epics::pvData::int32 payloadSize = 0) = 0;
...
public:
   ...
   * @return <code>true</code> if transport was granted, <code>false</code>
   * otherwise.
   */
  virtual bool
  acquire(std::tr1::shared_ptr<ClientChannelImpl> const &client) = 0;
   ...
   */
  virtual void
  setRemoteTransportReceiveBufferSize(std::size_t receiveBufferSize) = 0;
  ...
  virtual void setRemoteTransportSocketReceiveBufferSize(
      std::size_t socketReceiveBufferSize) = 0;
   ...
   */
  virtual void
  enqueueSendRequest(TransportSender::shared_pointer const &sender) = 0;
  ...
  /**
   * Waits (if needed) until transport is verified, i.e. verified() method is
   * being called.
   ...
   */
  virtual void
  authNZMessage(epics::pvData::PVStructure::shared_pointer const &data) = 0;
};
...
public:
  ...
  /**
   * Handle response.
   * @param[in] responseFrom  remote address of the responder, <code>0</code> if
   * unknown.
   * @param[in] transport response source transport.
   * @param[in] version message version.
   * @param[in] payloadSize size of this message data available in the
   * <code>payloadBuffer</code>.
   * @param[in] payloadBuffer message payload data.
   *                      Note that this might not be the only message in the
   * buffer. Code must not manipulate buffer.
   */
...
public:
   ...
   * @param payloadBuffer
   */
  virtual void response(Transport::shared_pointer const &transport,
                        epics::pvData::int8 version,
